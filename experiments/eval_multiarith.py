...
async def main():
    args = parse_args()
    result_file = None
    with open('datasets/MultiArith/test.json', 'r',encoding='utf-8') as file:
        dataset = json.load(file)
    dataset = multiarith_data_process(dataset)
    current_time = Time.instance().value or time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())
    Time.instance().value = current_time
    result_dir = Path(f"{AGP_ROOT}/result/multiarith")
    ...
    for i_batch in range(min(num_batches, args.num_iterations)):
        ...
        for task, answer, true_answer in zip(current_batch, raw_answers, answers):
            ...
            with open('eval_multiarith.json', 'a',encoding='utf-8') as file:
