...
from datasets.gsm8k_dataset import aqua_data_process,gsm_get_predict
...
def parse_args():
    parser = argparse.ArgumentParser(description="AGP Experiments on gsm8k")
...
async def main():
    args = parse_args()
    result_file = None
    dataset = JSONLReader.parse_file("datasets/AQuA/test.jsonl")
    dataset = aqua_data_process(dataset)
    current_time = Time.instance().value or time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())
    Time.instance().value = current_time
    result_dir = Path(f"{AGP_ROOT}/result/aqua")
    result_dir.mkdir(parents=True, exist_ok=True)
    result_file = result_dir / f"eval_{args.domain}_{args.llm_name}_{current_time}.json"
    ...
    for i_batch in range(min(num_batches, args.num_iterations)):
        ...
        for i_record, record in enumerate(current_batch):
            ...
            print(input_dict)
            answer_log_probs.append(asyncio.create_task(realized_graph.arun_evaluate(input_dict,args.num_rounds)))
        ...
        for task, answer, true_answer in zip(current_batch, raw_answers, answers):
            ...
            with open('eval_aqua.json', 'a',encoding='utf-8') as file:
