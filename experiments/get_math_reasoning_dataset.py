...
async def main():
    ...
    result_dir = Path(f"{AGP_ROOT}/result/gsm8k")
    ...
    for i_batch in range(min(num_batches, args.num_iterations)):
        ...
        for i_record, record in enumerate(current_batch):
            ...
            answer_log_probs.append(asyncio.create_task(realized_graph.arun_collecter(input_dict,args.num_rounds)))
        ...
        for task, answer, log_prob, true_answer, mask, edge_weight in zip(current_batch, raw_answers, log_probs, answers, masks, edge_weights):
